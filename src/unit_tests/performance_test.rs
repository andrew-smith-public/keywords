...
mod tests {
    ...
    async fn test_performance_with_debug() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let file_path = std::env::temp_dir()
            .join("test_performance_with_debug.parquet")
            .to_string_lossy()
            .to_string();
        ...
        for rg in 0..5 {
            ...
            for col_idx in 0..10 {
                ...
                // Remember 42,153rd row values from columns 0, 1, 2 in first row group
                if rg == 0 && col_idx < 3 {
                    remembered_values.push(values[42_153].clone());
                }
    ...
    async fn test_performance_with_debug_1_row_group() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Step 3: Create parquet file
        let file_path = std::env::temp_dir()
            .join("test_performance_with_debug_1_row_group.parquet")
            .to_string_lossy()
            .to_string();
    ...
    async fn test_performance_with_debug_v2_parquet() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Step 3: Create parquet file
        let file_path = std::env::temp_dir()
            .join("test_performance_with_debug_v2_parquet.parquet")
            .to_string_lossy()
            .to_string();
